...
Cache.configure = function configure(opts) {
  ...
  assert(opts.redis, 'you must pass a redis url in `options.redis`');

  if (typeof opts.redis === 'string') {
    // Entertain the old API which creates its own Redis client.
    redis = new FailoverRedis(opts.redis);
  }
  else {
    // Accept a passed Redis client.
    redis = opts.redis;
  }
...
Cache.get = function get(opts, callback) {
  ...
  Cache.getKey(opts, function(err, value) {
    ...
    Cache._getNoCache(opts, function(err, data) {
      if (err) {
        return callback(err);
      }
